...
namespace URWPGSim2D.Strategy
{
    public class Strategy : MarshalByRefObject, IStrategy
    {
        ...
        public Decision[] GetDecision(Mission mission, int teamId)
        {
            ...
            Vector3 assume3 = new Vector3(996f, 0f, 300f);      //假想点③-障碍①下
            Vector3 assume4 = new Vector3(0f, 0f, -370f);       //假想点④-障碍②上
            Vector3 assume5 = new Vector3(-1180f, 0f, 400f);    //假想点⑤-障碍③下
            Vector3 assume6 = new Vector3(-2050f, 0f, -990f);   //假想点⑥-障碍③上
            #endregion
            #region 洞点
            ...
            if (b0 == 0)
            {
                ...
                {//前往假定点③
                    FishDecision.FishDecision.Dribble2(ref decisions[0], mission.TeamsRef[0].Fishes[0], assume3, 10, 20, 30, 3, 7, 7, 7, 5, 100, true);
                }
                if (FishDecision.FishDecision.judgearea5(Trace, 2) == true)
                {//前往假定点④
                    FishDecision.FishDecision.Dribble2(ref decisions[0], mission.TeamsRef[0].Fishes[0], assume4, 10, 20, 30, 3, 7, 7, 7, 5, 100, true);
                }
                if (FishDecision.FishDecision.judgearea5(Trace, 3) == true)
                {//前往假定点⑤
                    FishDecision.FishDecision.Dribble2(ref decisions[0], mission.TeamsRef[0].Fishes[0], assume5, 30, 20, 10, 3, 7, 7, 7, 5, 100, true);
                }
                if (FishDecision.FishDecision.judgearea5(Trace, 4) == true)
                {//前往假定点⑥
                    FishDecision.FishDecision.Dribble2(ref decisions[0], mission.TeamsRef[0].Fishes[0], assume6, 30, 20, 10, 7, 7, 7, 7, 5, 100, true);
                }
                if (FishDecision.FishDecision.judgearea5(Trace, 5) == true)
                {//前往洞点
                 /*提高预测点坐标*/
                    FishDecision.FishDecision.Dribble_Will(mission, ref decisions[0], 0, 0, 0, hole, 30, 45, 60, 3, 2, 1);
                }
                if (FishDecision.FishDecision.judgearea5(Ball_1, 6) == true && FishDecision.FishDecision.judgearea5(Trace, 7) == true)
                {//前往①号得分点
                 /*缩小角度后半段冲刺*/
                ...
                if (FishDecision.FishDecision.judgearea5(Ball_1, 6) == true)
                {
                    FishDecision.FishDecision.Dribble_Will(mission, ref decisions[0], 0, 0, 0, keyPoint, 30, 45, 60, 3, 2, 1);
                }
            }
            /**/
            if (b0 == 1)
